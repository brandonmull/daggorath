...
One repository holds two packages. The split survives not as a repo boundary but as an **import boundary** — `daggorath_gym` imports only `gymnasium` and `numpy`; `daggorath_agent` imports the training stack. Keeping them as separate packages preserves that boundary without the friction of two repos:

- **`gym/`** (`daggorath-gym`) — the environment. Depends on `gymnasium` and `numpy` only; imports no training library.
- **`agent/`** (`daggorath-agent`) — the training harness. Depends on `stable-baselines3`, `torch`, and `sb3-contrib`; consumes `daggorath_gym` as a library.

Both install editable from the repo root, so the environment stays free of training dependencies while the training code — algorithms, sweeps, checkpoints — evolves without touching the environment.
...
- The reward wrapper stays in the gym package: it is gymnasium-only and algorithm-agnostic — task definition, not a trainer.
- Joint action masking lives in the agent package as a custom policy; the environment keeps the no-op fallback for invalid pairs.
