...
The environment samples objects in three passes, matching the three object channels. The two hands come from the `leftHand` and `rightHand` pointers (0x021D:0x021E, 0x021F:0x0220). The pack comes from a chain walk of `firstPackObject` (0x0229:0x022A), a LIFO stack. Floor objects come from an **arena scan** — a linear 14-byte-stride walk of the object array from 0x0B15 to `nextObjSlot` (0x020F:0x0210) — because the game itself finds floor objects that way: `GetNextOject` (CF63) "walks the objects in memory without looking at their chain pointers," and `CmdGET` (D522) loops `GetObjectAtCoor` (CF53), which is `GetNextOject` plus a coordinate and floor-location filter. The chain pointers (slot + 0:1) link only the pack, the two hands, and each creature's held objects — never floor objects. Monster-held objects (location FF) stay hidden.

Each object is reported as its class (slot + 10) plus, when revealed, its proper type (slot + 9) — reveal is the strength-to-reveal field (slot + 11) being 0. Floor objects are filtered to the current level (slot + 4 equals the current maze level) and carry their X/Y (slot + 3, slot + 2). The lit torch's minutes (slot + 6) are self-state, exposed at full precision.

### Wire

Objects ship in one fixed-size `O` record: the two hands, then the pack, then the floor objects, then the lit torch — each a fixed-capacity sub-array, empty slots marked by a sentinel. Every carried entry is three raw bytes — class (slot + 10), proper type (slot + 9), reveal threshold (slot + 11) — and every floor entry adds X and Y (absolute). The sentinel is a class byte of `0xFF` (real classes are 0–5), so an empty slot is all-`FF`. Python derives both faces from those bytes: the perceived specifier index for the observation, and the true type + reveal threshold for the reward's internal state. The derivation is a drop, not a computation — the six class bytes 0–5 are themselves the bare-class specifier indices, so an unrevealed object's index is simply its class byte (the proper type is dropped), while a revealed object's index is its full "proper name + class" specifier, reached through one flat table from proper-type token to specifier index, built at import. The perceived specifier is the shared index the observation and the action's `object` slot both use.

The `O` record is 76 bytes: two hands (2 × 3), eight pack slots (8 × 3), eight floor slots (8 × 5), and a six-byte lit-torch entry (identity + minutes + physical light + magic light). The caps are fixed-capacity sampling bounds — the game imposes no pack or floor limit (boot allocates 65 objects, most carried by creatures) — so the record truncates the pack walk at eight and the floor scan at eight, in chain and array order respectively. Truncation is a POC bound, revisited if a run shows more than eight objects matter at once.
...
- **Torch minutes are true-state.** The torch's remaining minutes and light (slots + 6/7/8 of the lit torch) live in the lit-torch object record, not the perceived scalars — the player tracks torch life via dimming (the effective light), and the reward's sight potential reads the minutes from true state, not `ambient_light` (which jumps on the Wizard's death). Not curriculum-removable.
...
- **Floor objects are found by arena scan, not chain walk.** The chain pointers link only the pack, hands, and monster-held lists; floor objects live in the flat array and are found by the linear stride walk the game itself uses (`GetNextOject` / `GetObjectAtCoor`). The wire ships the first eight floor objects on the current level, in array order.
