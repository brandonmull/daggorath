...
| Command rejected | "???" in the command area; `command_parser_matched_exactly` (0x027B) never fires |
...
- **Environment reports, wrapper prices.** Detection is a fact (the environment's job, from `command_area_text`/`command_parser_matched_exactly`); sequencing and valuation are the reward wrapper's job. `command_rejected` would be a true-state fact, not a `PERCEIVED_SPACE` channel — the policy never sees it, only feels the penalty.
...
- **Detection fork (unresolved).** Persistent transitions (kill `FF→0`, reveal `→0`, descent, death) can be edge-detected in Python from raw state deltas — consistent with "wire carries raw, Python derives." Transient signals (`command_parser_matched_exactly` `0→FF→0`, `!!!`, one-frame sounds) only Lua sees frame-accurately, and would need a new `E` record. `command_rejected` is persistent ("???" lingers), so Python can derive it.
