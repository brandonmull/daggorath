...
>
> **on causal structure** — "causal structure is primary knowledge worth creating a foundation on. it should be orthogonal but related with all other knowledge, i believe. i don't think that causal structure makes up all there is to a world model, however."
>
> **on skill** — "a skill is something you can do well, that may or may not depend heavily on knowledge. … i could consider a skill to be something that is executed post-reasoning — what skill to employ is exactly the thing that is selected by reasoning."
...
> **the refinement** — "i really think of knowledge as some preliminary, reusable form that can combined with some other set of learned data to produce actionable weights, per task."
...
> **the request** — "let's consider our notion of 'composable' here by inspecting our concrete example of the causal chain for torch lighting, and seeing if we can discover the hidden attributes behind each link that actually makes them 'composable' with one another."
