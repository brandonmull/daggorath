...
- **Joint action masking.** The action space is `MultiDiscrete([26, 31])` — a verb form and an object specifier. The INCANT verb form is valid only with the nine ring specifiers. SB3's per-axis masking cannot express that cross-axis constraint, so a joint-mask policy belongs here as a custom policy. It is deferred: the environment's `derive_command_index` returns `None` for an invalid INCANT pair and `step()` treats it as a no-op, so plain PPO runs correctly and merely wastes the occasional step. Only about 2.7% of the 806 potential choices are invalid.