...
This document records the training curriculum — the staged path from a fresh agent to one that plays competently. The curriculum has two axes, which are independent levers and are documented separately below:

- **Perception scaffolding** — information the environment exposes now and removes later, so the agent internalizes the skill. This is the original scope of this document.
- **Objective staging** — which reward channels are active and which commands are available, progressing through a fixed ladder. This is the newer scope and the primary design below.
...
One source drives every curriculum effect: the **novelty-flag memory** (see `reward/plan.md`). A flag records the unknown → known transition — the first reveal of an object, the first entry into a cell, the first sighting of a creature type. At the moment a flag flips, three things can happen at once:
...
| Axis | What is staged | Mechanism | Where |
|------|---------------|-----------|-------|
| Perception scaffolding | information exposed to the agent | fields added, then removed | environment |
| Objective staging | reward channels + command availability | `stage` parameter on the reward; mask in the policy | reward + trainer |

The perception scaffolding items (strength, reveal threshold, sound→source, map memory) are unchanged and remain "exposed now, removed later." Objective staging is the addition.

## Open Questions

- **Graduation.** How a stage is judged complete before advancing — a performance threshold, a fixed number of timesteps, or manual judgment — is deferred. The first trainer advances stages by hand (`--stage`), with an automatic trigger as a follow-up.
- **Mask granularity.** Gating whole templates (EXAMINE, LOOK) is simple per-axis masking on the template axis. Gating object-level joint rules (INCANT + ring-only) is the separate, already-deferred joint-mask case and is not conflated here.
- **Where the unlock set lives.** It is episode-scoped progress, so it is reward-side bookkeeping alongside the novelty memory — not perception, and never in the observation.

## Reference Documents

| Document | What It Contains |
|----------|-----------------|
| `docs/plans/reward/plan.md` | The reward layers, potentials, and the novelty memory |
| `docs/plans/state/plan.md` | The scalar fields the potentials draw from |
| `docs/plans/objects/plan.md` | Object attainment and the reveal field |
| `docs/plans/perception/plan.md` | The perception, which carries no novelty flags |
| `docs/findings/combat-model.md` | The survival margin and the fight/run foundation |